 ...
 * @param {string} [options.image]    Social image path or absolute URL. When
 *                                    omitted, Next's file convention injects
 *                                    the generated card from
 *                                    app/opengraph-image.jsx instead.
...
export function buildMetadata({
  ...
  // Only declare images when the route supplies its own. Otherwise the
  // generated card wins by convention, and hard-coding a size here would let
  // og:image:width drift out of step with the real file.
  ...
  return {
    ...
    description,
    keywords,
    // A page excluded from indexing should not also assert a canonical URL for
    // itself; the two directives pull in opposite directions.
